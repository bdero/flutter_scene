...
highp vec4 IrradianceCageTap(vec3 offset, highp vec3 base_lattice, vec3 alpha,
                             highp vec3 biased_position, vec3 direction) {
  highp vec3 counts = frag_info.gi_counts.xyz;
  highp vec3 lattice = base_lattice + offset;
  // TODO(gi-far-origin): subtract anchor on CPU and upload relative lattice
  // to avoid fp32 precision loss far from origin.
  highp vec3 relative = lattice - frag_info.gi_anchor.xyz;
  float inside =
  ...
  highp vec3 probe_position = lattice * frag_info.gi_grid.xyz;
  highp vec3 to_probe = probe_position - biased_position;
  highp float probe_distance = length(to_probe);
  ...
  highp float index = ProbeLinearIndex(ProbeWrapSlot(lattice, counts), counts);
  highp vec2 inverse_atlas = frag_info.gi_atlas.zw;

  if (frag_info.gi_visibility.x > 0.0) {
    highp vec2 depth_tile = ProbeTileOrigin(index, frag_info.gi_counts.w,
    ...
    highp float scale = frag_info.gi_visibility.z;
    highp vec2 moments = texture(irradiance_field,
                           ProbeAtlasUv(-probe_direction, depth_tile,
                                        kProbeDepthInterior, inverse_atlas))
                       .rg;
    highp float mean = moments.x * scale;
    highp float mean_square = moments.y * scale * scale;
    // Filtering pushes the difference slightly negative, hence the abs.
    highp float variance = abs(mean * mean - mean_square);
    highp float excess = probe_distance - mean - frag_info.gi_visibility.y;
    highp float chebyshev =
...
float IrradianceFieldCoverage(highp vec3 world_position) {
  if (frag_info.gi_grid.w <= 0.0) return 0.0;
  highp vec3 lattice = world_position / frag_info.gi_grid.xyz;
  highp vec3 relative = lattice - frag_info.gi_anchor.xyz;
  highp vec3 inset = min(relative, frag_info.gi_counts.xyz - vec3(1.0) - relative);
  highp float edge = min(inset.x, min(inset.y, inset.z));
...
highp vec3 SampleIrradianceField(highp vec3 world_position, vec3 direction,
                                 vec3 view) {
  highp vec3 spacing = frag_info.gi_grid.xyz;
  highp float cell = min(spacing.x, min(spacing.y, spacing.z));
  // The 2021 unified self-shadow bias, expressed as a fraction of the cell
  // edge so one default holds across scene scales.
  highp vec3 biased_position =
      world_position +
      (direction * 0.2 + view * 0.8) * (0.75 * cell * frag_info.gi_anchor.w);
  highp vec3 lattice = biased_position / spacing;
  highp vec3 base = floor(lattice);
  ...
  highp vec3 result = total.a > 1e-6 ? total.rgb / total.a : vec3(0.0);
