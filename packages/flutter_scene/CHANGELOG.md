...
* Material fragment shaders default to mediump with explicit highp on positions, coordinates, depth, and the HDR accumulators (see `shaders/PRECISION.md`), so GPUs that run half precision faster (Mali, Adreno, the web) shade the standard and physical materials at their fp16 rate. Metal is unaffected. A `.fmat` body inherits the default and should declare `highp` on positions or coordinates it computes itself.
