...
* Update `flutter_scene-idioms` skill (v8) with point light shadows, the runtime SMAA/TAA tuning surface, the automatic quality ladder, render introspection, `SceneTickListener`, `FlyCameraController.setMoveInput`, and game input through `flutter_scene_input`.
...
* BREAKING for classes that `implements` rather than extends these types: `Geometry` gained `emitsStandardVaryings` and `debugEdges`, `Material` gained `depthCompare` and `participatesInDebugViews`, `Camera` gained `projectToScreenUv`, and `ComponentCodec` gained `writeLiveProperty`, all with default implementations. `IrradianceFieldBakeStepper` takes a required `pointShadowFrame`, and its `renderView` callback receives it.
