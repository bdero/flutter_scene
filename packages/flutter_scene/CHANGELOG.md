...
* UI layout, through a `canvas` component and a `rectTransform` on what sits inside it. A canvas is a rectangle, screen-space or in the scene, that children position themselves against by anchors and a pivot rather than by coordinates; two anchors at one point hold a size, two apart stretch. `layOutCanvas` solves a subtree, `solveRect` is the whole of the arithmetic, and both are GPU-free.
