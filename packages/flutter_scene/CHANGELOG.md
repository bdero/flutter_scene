...
* `PointerLock` hides and holds the mouse cursor and reports per-frame `movement` for first-person look, on macOS, Windows, Linux (X11 and Wayland), and the web, with no plugin or package to add.
