...
String emitFragmentGlsl(FmatMaterial material) {
  ...
  if (material.engineInputs.contains('filtered_scene_color')) {
    sb.writeln('// Compact roughness-filtered scene-color atlas.');
    sb.writeln('uniform sampler2D scene_filtered_color;');
    sb.writeln('float TransmissionWeight0(float x) {');
    sb.writeln('  return (x * (x * (-x + 3.0) - 3.0) + 1.0) / 6.0;');
    sb.writeln('}');
    sb.writeln('float TransmissionWeight1(float x) {');
    sb.writeln('  return (x * x * (3.0 * x - 6.0) + 4.0) / 6.0;');
    sb.writeln('}');
    sb.writeln('float TransmissionWeight2(float x) {');
    sb.writeln('  return (x * (-3.0 * x * x + 3.0 * x + 3.0) + 1.0) / 6.0;');
    sb.writeln('}');
    sb.writeln('float TransmissionWeight3(float x) {');
    sb.writeln('  return x * x * x / 6.0;');
    sb.writeln('}');
    sb.writeln('vec3 SampleTransmissionBand(');
    sb.writeln('    vec2 uv, float level, vec2 scene_size, vec2 atlas_size) {');
    sb.writeln(
      '  vec2 band_size = max(floor(scene_size / exp2(level)), vec2(1.0));',
    );
    sb.writeln(
      '  float offset_x = scene_size.x - '
      'ceil(scene_size.x / exp2(level - 1.0));',
    );
    sb.writeln('  vec2 coord = uv * band_size + 0.5;');
    sb.writeln('  vec2 base = floor(coord);');
    sb.writeln('  vec2 f = fract(coord);');
    sb.writeln(
      '  vec2 g0 = vec2(TransmissionWeight0(f.x) + '
      'TransmissionWeight1(f.x), TransmissionWeight0(f.y) + '
      'TransmissionWeight1(f.y));',
    );
    sb.writeln(
      '  vec2 g1 = vec2(TransmissionWeight2(f.x) + '
      'TransmissionWeight3(f.x), TransmissionWeight2(f.y) + '
      'TransmissionWeight3(f.y));',
    );
    sb.writeln(
      '  vec2 h0 = -vec2(1.0) + vec2(TransmissionWeight1(f.x), '
      'TransmissionWeight1(f.y)) / g0;',
    );
    sb.writeln(
      '  vec2 h1 = vec2(1.0) + vec2(TransmissionWeight3(f.x), '
      'TransmissionWeight3(f.y)) / g1;',
    );
    sb.writeln('  vec2 origin = vec2(offset_x, 0.0);');
    sb.writeln('  vec2 minimum = origin + vec2(0.5);');
    sb.writeln('  vec2 maximum = origin + band_size - vec2(0.5);');
    sb.writeln(
    ...
    sb.writeln(
      ...
      'g1.x * texture(scene_filtered_color, p1).rgb) + g1.y * '
      '(g0.x * texture(scene_filtered_color, p2).rgb + '
      'g1.x * texture(scene_filtered_color, p3).rgb);',
    );
    sb.writeln('}');
    sb.writeln('// Samples the accumulated scene color with perceptual');
    sb.writeln('// roughness and IOR-dependent blur.');
    sb.writeln(
    ...
    sb.writeln('  vec3 sharp = texture(scene_opaque_color, uv).rgb;');
    sb.writeln('  float bands = frag_info.transmission_info.y;');
    sb.writeln(
      '  if (frag_info.transmission_info.x < 0.5 || bands < 1.0) '
      'return sharp;',
    );
    sb.writeln(
    ...
    sb.writeln('  if (lod <= 0.0) return sharp;');
    sb.writeln('  vec2 atlas_size = 1.0 / frag_info.transmission_info.zw;');
    sb.writeln('  float lo = max(floor(lod), 1.0);');
    sb.writeln('  float hi = min(lo + 1.0, bands);');
    sb.writeln(
      '  vec3 lo_color = SampleTransmissionBand('
      'uv, lo, scene_size, atlas_size);',
    );
    sb.writeln(
      '  vec3 hi_color = SampleTransmissionBand('
      'uv, hi, scene_size, atlas_size);',
    );
    sb.writeln('  vec3 filtered = mix(lo_color, hi_color, fract(lod));');
    sb.writeln('  return mix(sharp, filtered, min(lod, 1.0));');
    sb.writeln('}');
    sb.writeln();
  }
