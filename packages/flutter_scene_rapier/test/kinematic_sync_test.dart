...
void _stepWithFixedUpdate(Node root, RapierWorld world, double dt) {
...
void main() {
  test('moving a kinematic body node updates the native body', () async {
    ...
    expect(body.readNativeTranslation().y, closeTo(0.0, 1e-5));

    node.localTransform = Matrix4.translation(Vector3(0, 3, 0));
    _stepWithFixedUpdate(root, world, 1.0 / 60.0);

    expect(body.readNativeTranslation().y, closeTo(3.0, 1e-3));
  });
  ...
  test('kinematic body pushes a dynamic ball it contacts', () async {
    ...
    final paddle = RapierRigidBody(type: BodyType.kinematic);
    paddleNode.addComponent(paddle);
    paddleNode.addComponent(
      RapierCollider(shape: BoxShape(halfExtents: Vector3(2, 0.25, 2))),
    );
    root.add(paddleNode);
    paddle.mount();
    paddleNode.getComponents<RapierCollider>().first.mount();

    // Dynamic ball resting just above the paddle.
    final ballNode = Node(
      localTransform: Matrix4.translation(Vector3(0, 1.0, 0)),
    );
    final ball = RapierRigidBody(type: BodyType.dynamic_, mass: 1.0);
    ...
    expect(ball.readNativeTranslation().y, greaterThan(2.5));
  });
}
