...
Status (branch `bdero/renderer`): items 1, 2, 5, 6, 7, 8 done; item 3 half done (analytic directional light landed, shadow map still pending); items 4, 9 remain. Everything done so far is compile-verified and smoke-run on macOS + Impeller; visual correctness still wants an on-device pass.
...
3. **Directional light + single shadow map.** Analytic directional light: ✅ Done. `DirectionalLight` (direction, color, intensity) + a `Lighting` context handed to `Material.bind`; the standard fragment shader layers a Cook-Torrance direct term on the IBL ambient. Shadow map: still pending — needs an orthographic light-space frustum, a depth-only `ShadowPass` on the graph (a `ShadowEncoder` to re-walk the scene with a depth-only shader, since `Node.render` is typed to `SceneEncoder`), and manual PCF + normal-offset bias in the standard fragment shader (Flutter GPU has no depth bias state or depth-compare samplers). `DirectionalLight.castsShadow` exists but is currently a no-op.
