...
Status (branch `bdero/renderer`): items 1–8 done; item 9 remains. Everything compiles, `dart analyze` is clean, and the example app was eyeballed on macOS + Impeller — the render-graph refactor, PBR-Neutral tone mapping, multiscatter IBL, directional light + shadow map (logo casting a shadow on a ground plane), and the HDR pipeline all render correctly. The only thing visibly affected is the Toon `ShaderMaterial` example, whose output now passes through the engine tone-map (slightly compressed highlights) — see the `MATERIALS.md` TODO below. (An errant `flutter clean` mid-A4 left the example app's gitignored, hook-generated `build/models/` and `build/shaderbundles/example.shaderbundle` not regenerating; both were restored manually — via the importer CLI and `impellerc` respectively — so the example builds again, but the underlying "why didn't the example's build hook re-run after a clean" is unresolved.)
