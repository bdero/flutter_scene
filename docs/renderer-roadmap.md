...
Status (branch `bdero/renderer`): items 1, 2, 3, 5, 6, 7, 8 done; items 4 and 9 remain. Everything done so far is compile-verified and run on macOS + Impeller; the directional light + shadow map were eyeballed (logo casting a shadow on a ground plane), the rest still wants an on-device visual pass.
