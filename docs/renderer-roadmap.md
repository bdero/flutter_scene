...
Status (branch `bdero/renderer`): items 1–8 done; item 9 remains. Everything compiles, `dart analyze` is clean, the example runs on macOS + Impeller without errors; the directional light + shadow map were eyeballed (logo casting a shadow on a ground plane); the HDR pipeline (item 4) builds and runs but wasn't visually scrutinized; the rest still wants an on-device visual pass. (An errant `flutter clean` mid-A4 left the example app's gitignored, hook-generated `build/models/` and `build/shaderbundles/example.shaderbundle` not regenerating; both were restored manually — via the importer CLI and `impellerc` respectively — so the example builds again, but the underlying "why didn't the example's build hook re-run after a clean" is unresolved.)
