...
Status (branch `bdero/renderer`): **Phase A is done** (items 1–9), plus a follow-up lighting/material API cleanup (A9c, see below). Everything compiles, `dart analyze` is clean, and the example app was eyeballed on macOS + Impeller — the render-graph refactor, PBR-Neutral tone mapping, multiscatter IBL, directional light + shadow map (logo casting a shadow on a ground plane), the HDR pipeline, the runtime prefiltered-specular atlas, and the procedural studio default all render correctly. (An errant `flutter clean` mid-A4 left the example app's gitignored, hook-generated `build/models/` and `build/shaderbundles/example.shaderbundle` not regenerating; both were restored manually — via the importer CLI and `impellerc` respectively — so the example builds again, but the underlying "why didn't the example's build hook re-run after a clean" is unresolved.)

The **API cleanup (A9c)** that landed alongside item 9 (breaking changes — small consumer base, worth getting right):
- Removed the `Environment` wrapper class. Image-based lighting is now `Scene.environment` (an `EnvironmentMap`, defaulting to `EnvironmentMap.studio()`) plus `Scene.environmentIntensity` (a scalar). A `PhysicallyBasedMaterial.environment` per-material override is an `EnvironmentMap?`.
- Tone mapping + exposure moved to the `Scene`: `Scene.exposure` (**default `1.0`** — the old `2.0` was a hack compensating for a buggier renderer) and `Scene.toneMapping` (`ToneMappingMode`, default `pbrNeutral`). `Environment.exposureFromPhysicalCamera` → `Scene.physicalCameraExposure(...)`.
- `EnvironmentMap` now always carries a prefiltered-radiance atlas + diffuse SH-9 (no nullable getters). The legacy pre-baked-irradiance-texture path and the raw-radiance-LOD fallback are gone, so the standard shader has one diffuse path (SH) and one specular path (atlas) — `radiance_texture` / `irradiance_texture` samplers and the `use_diffuse_sh` / `use_prefiltered_radiance` flags are removed; `FragInfo` shrank from 84 to 80 floats. `EnvironmentMap.empty()` is a black atlas + zero SH; `fromAssets` / `fromUIImages` dropped their `irradianceImage` params; `fromGpuTextures` now takes a prefiltered atlas (+ optional SH). `kEnvironmentMultiplier=2.0` and the `pow(1.02-roughness,12)` blend are gone.
- `prefilterEquirectRadiance` and the `kPrefilterBand*` constants are exported (`lib/src/render/env_prefilter.dart`) for advanced use.
- Examples migrated to the new API; the Toon `ShaderMaterial` output still passes through the engine tone-map (slightly compressed highlights) — `MATERIALS.md` still needs updating, and custom shaders that used `useEnvironment` now get `prefiltered_radiance` + `brdf_lut` (not the old `radiance_texture` / `irradiance_texture`).
...
2. **SH-9 diffuse irradiance.** ✅ Done. `EnvironmentMap.computeDiffuseSphericalHarmonics(ui.Image)` projects an equirect radiance image onto SH-9 (cosine convolution + 1/pi folded in); `fromAssets`/`fromUIImages` compute it from the radiance image when no irradiance image is given. The standard fragment shader evaluates it (`EvaluateDiffuseSH`, gated by `use_diffuse_sh` in FragInfo) and skips the irradiance texture + `2.0` fudge on that path. A separate `ShInfo` UBO failed to bind under Impeller, so the coefficients live as individual `vec4` members in FragInfo. The bundled `royal_esplanade` default still uses its irradiance texture; SH kicks in for user-supplied radiance-only environments. (Note: implemented as runtime computation, not via the glTF importer as the original wording suggested.)
3. **Directional light + single shadow map.** ✅ Done. `DirectionalLight` (direction, color, intensity, shadow knobs) + a `Lighting` context handed to `Material.bind`; the standard fragment shader layers a Cook-Torrance direct term on the IBL ambient. When `castsShadow` is set, a `ShadowPass` renders scene depth from an orthographic light-space frustum into an fp16 depth-in-`.r` map (a `ShadowEncoder` re-walks the scene via the new `SceneDrawList` interface paired with the `DepthOnlyFragment` shader), and the standard fragment shader samples it with 3×3 PCF + normal-offset bias (Flutter GPU has no depth-bias state or depth-compare samplers). Note: render-graph passes each own their command buffer now — Flutter GPU's `RenderPass` holds a live Metal encoder, so a command buffer hosts at most one render pass. Sampling the render-to-texture shadow map needs a V-flip. Bias values may want tuning on real content.
4. **HDR offscreen pipeline.** ✅ Done. `ScenePass` renders into an `r16g16b16a16Float` color target (MSAA resolved in linear HDR); a full-screen `TonemapPass` (new `FullscreenVertex` + `TonemapFragment` shaders, a static NDC-quad buffer) reads it, un-premultiplies, applies exposure + the tone-mapping operator + the display EOTF, re-premultiplies, and writes the 8-bit swapchain. The standard/unlit fragment shaders stop tone-mapping and output linear HDR premultiplied by alpha (the unlit shader now linearizes its sRGB base color too). `Surface` is simplified: `getNextRenderTarget` → `getNextSwapchainColorTexture` (the swapchain is plain 8-bit non-MSAA; MSAA lives on the HDR pass). Verified on macOS: scenes render right-side up, AA edges stay crisp (MSAA now on the HDR pass), translucency composites correctly. **`ShaderMaterial` authors who tone-mapped in their own shader must stop — the engine tone-maps now; `MATERIALS.md` needs updating.**
...
9. **Runtime PMREM-style prefilter path** (three.js approach, 2D-atlas variant, fragment passes only) for user-supplied radiance maps, plus a **procedural default studio environment** so zero-config looks good. ✅ Done. `prefilterEquirectRadiance` GGX-importance-samples an equirect radiance texture into a vertical roughness-band atlas (`kPrefilterBandCount` bands of `512×256`, `r16g16b16a16Float`) in one full-screen pass (`flutter_scene_prefilter_env.frag` + `env_prefilter.dart`), run once at `EnvironmentMap` construction; the standard shader samples it via `SamplePrefilteredRadiance` (band lerp + V-flip). `EnvironmentMap.studio()` generates a procedural studio equirect on the CPU (cool ceiling → neutral horizon → warm floor + a broad top fill + soft key/fill lobes), uploads it, and runs the SH-9 projection + prefilter on it; it's the zero-config `Scene.environment` default. `royal_esplanade.png` stays bundled for `EnvironmentMap.fromAssets`. (Phase B upgrades: HDR-linear procedural env source, separable-Gaussian or VNDF prefilter, a full-res mirror band — see Phase B.)
...
- ~~Default environment: keep `royal_esplanade` or switch to a neutral studio?~~ Resolved (A9b): the procedural `EnvironmentMap.studio()` is the default; `royal_esplanade.png` is still bundled and reachable via `EnvironmentMap.fromAssets`.
